...
import client.opennight.event.impl.PrePacketEvent;
...
public class ConnectionPatch extends ClientBase {
    public static boolean onPacketReceive(Packet<?> packet) {
        if (mc == null || mc.level == null || mc.player == null || packet == null || !NightClient.isReady()) {
            return false;
        }
        PrePacketEvent prePacket = new PrePacketEvent(packet);
        NightClient.getInstance().getEventBus().call(prePacket);
        ...
        NightClient.getInstance().getEventBus().call(receiveEvent);
