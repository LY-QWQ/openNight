...
import client.opennight.event.impl.ChatReceiveEvent;
import client.opennight.hud.TabListInfo;
import client.opennight.modules.impl.render.Watermark;
...
public class PlayerTabOverlayPatch extends ClientBase {
    ...
    public static void onRenderPre(PlayerTabOverlay overlay, GuiGraphics graphics, int width, Scoreboard scoreboard, Objective objective, CallbackInfo callbackInfo) {
        ...
        if (!NightClient.isReady() || NightClient.getInstance().getModuleManager() == null) return;
        Watermark watermark = (Watermark) NightClient.getInstance().getModuleManager().getModule(Watermark.class);
