...
import client.opennight.event.impl.*;
...
import client.opennight.utils.game.PlayerUtil;
...
public class LivingEntityPatch {
    ...
    private static boolean shouldFastDig(LivingEntity entity) {
        if (!NightClient.isReady() || entity != ClientBase.mc.player) return false;
    ...
    public static void onTravel(LivingEntity entity, Vec3 movement, CallbackInfo callbackInfo) throws Exception {
        if (entity == null || entity != ClientBase.mc.player || !NightClient.isReady()) return;
    ...
    public static void onHurt(LivingEntity entity, DamageSource source, float amount, CallbackInfo callbackInfo) {
        if (NightClient.isReady()) {
            NightClient.getInstance().getEventBus().call(new EntityHurtEvent(entity, source, amount));
        }
    }
}
