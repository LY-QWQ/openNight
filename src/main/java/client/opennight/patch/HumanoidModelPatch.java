...
public class HumanoidModelPatch {
    public static CameraPitchEvent onPitchRender(LivingEntity entity, float pitch) {
        if (NightClient.isReady() && entity == ClientBase.mc.player && ClientBase.mc.level != null) {
            return (CameraPitchEvent) NightClient.instance.getEventBus().call(new CameraPitchEvent(pitch));
        }
