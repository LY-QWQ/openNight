...
import client.opennight.event.impl.PostMotionEvent;
...
import client.opennight.modules.impl.render.ESP;
...
public class MinecraftPatch {
    ...
    public static void onTick(Minecraft minecraft, CallbackInfo callbackInfo) throws Throwable {
        if (!initialized) {
            synchronized (MinecraftPatch.class) {
                if (!initialized) {
                    ClientBase.mc = NightClient.getMcInstance();
        ...
        if (NightClient.isReady()) {
            ...
            NightClient.instance.getEventBus().call(new TickEvent());
        }
    }
    ...
    public static void onSetLevel(ClientLevel level, CallbackInfo callbackInfo) {
        if (NightClient.isReady()) {
            NightClient.getInstance().getEventBus().call(new DisconnectEvent());
        }
    }
    ...
    public static boolean onShouldEntityGlow(Minecraft minecraft, Entity entity, Invocation<Entity, Boolean> original) throws Exception {
        if (NightClient.isReady()
                && NightClient.instance.getModuleManager() != null
                && ESP.INSTANCE != null
