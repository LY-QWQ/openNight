 ...
 * jar onto the Forge GameClassLoader, so {@link NightClient} and all 21 patch
 * handler classes share Minecraft's class loader.
 *
 * <p>This entry point intentionally does <b>not</b> construct {@link NightClient}.
 ...
 * NightClient} on the next tick. Letting the existing tick-driven lazy-init run
 * keeps the DLL path and the mod path identical from {@code NightClient.init}
 * onwards.</p>
 */
public final class ClientInit {
    ...
    public static synchronized void start(String extractedJarPath) {
        ...
        try {
            ...
            LOGGER.info("NightClient loader     = {}", NightClient.class.getClassLoader());
            ...
            // performs the real NightClient construction via its existing
            // lazy-init path.
            NightClient.registerPatches();
            ClassAgent.installPatchesAndRetransform();

            LOGGER.info("ClientInit done. NightClient will be constructed on the next Minecraft.tick.");
        } catch (Throwable t) {
