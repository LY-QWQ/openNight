package client.opennight.utils.rotation;

import client.opennight.NightClient;
import net.minecraft.network.protocol.game.ServerboundChatPacket;
import net.minecraft.network.protocol.game.ServerboundMovePlayerPacket;
import client.opennight.ClientBase;
import client.opennight.event.impl.CameraPitchEvent;
import client.opennight.event.impl.ChatEvent;
import client.opennight.event.impl.FallFlyingEvent;
import client.opennight.event.impl.RotationAnimationEvent;
import client.opennight.event.impl.JumpMarkerEvent;
...
import client.opennight.utils.animation.TickTimer;
...
public class RotationHandler
    ...
    public void onMotion(MotionEvent e) {
        if (e.isPost()) {
            if (mc.player != null && mc.player.tickCount <= 1 && NightClient.isReady()) {
                NightClient.getInstance().getEventBus().call(new WorldChangeEvent());
            }
            if (mc.player == null) {
                return;
            }
