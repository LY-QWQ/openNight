...
- **包名**: `client.opennight` (原 `shit.zen`)
- **构建**: Gradle + ForgeGradle，含自定义混淆任务（重命名所有 `client.opennight.*` 为随机 16 字符）
- **注入链**: runtime.dll → DllMain → JNI → Agent_OnAttach → ModuleInit → Bootstrap.init() → SRG mapping → patch 注册

## 目录结构

### `src/main/java/client/opennight/`
...
### `native/`
C++ native 代码：
- `dll/` - OpenNIGHT.dll (DLL 注入, JNI 桥接)
- `loader/` - OpenNIGHTLoader.exe (注入器, Qt6 GUI)
- 构建: CMake + vcpkg

## 模块系统

### 基类
- `client.opennight.modules.Module` extends `ClientBase`
...
## Patch 列表 (client.opennight.patch)
...
## 事件类型 (client.opennight.event.impl)
...
## 构建注意事项
- 混淆任务 (`obfuscateClasses`) 在 `reobfJar` 后运行，重命名 `client/opennight/**` 和 `asm/patchify/**`
